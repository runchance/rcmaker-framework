...
class Query extends BaseQuery
{
     ...
     * @return $this|Fetch
     */
    public function fetchSql(bool $fetch = true,$Builder=null)
    {
        $this->options['fetch_sql'] = $fetch;

        if ($fetch) {
            return new Fetch($this,$Builder);
        }
     ...
     * @throws Exception
     */
    public function buildSql(bool $sub = true,$Builder=null,$real=true): string
    {
        return $sub ? '( ' . $this->fetchSql(true,$Builder)->select(null,$real) . ' )' : $this->fetchSql(true,$Builder)->select(null,$real);
    }
    ...
    {
        if (empty($this->pk)) {
            $this->pk = $this->connection ? $this->connection->getPk($this->getTable()) : '';
        }
