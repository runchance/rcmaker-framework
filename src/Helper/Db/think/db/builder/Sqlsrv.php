...
namespace RC\Helper\Db\think\db\builder;

use RC\Helper\Db\think\db\Builder;
use RC\Helper\Db\think\db\exception\DbException as Exception;
use RC\Helper\Db\think\db\Query;
use RC\Helper\Db\think\db\Raw;
...
class Sqlsrv extends Builder
{
     ...
     * @var string
     */
    protected $selectSql = 'SELECT T1.* FROM (SELECT RC.*, ROW_NUMBER() OVER (%ORDER%) AS ROW_NUMBER FROM (SELECT %DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING%) AS RC) AS T1 %LIMIT%%COMMENT%';
    /**
