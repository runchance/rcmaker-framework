...
namespace RC\Helper\Cache\think\cache\driver;

use RC\Helper\Cache\think\cache\Driver;
    ...
    public function __construct(array $options = [])
    {
        ...
        if (extension_loaded('redis')) {
            ...
            if(is_array($this->options['host'])){
                ...
                $read_timeout = $this->options['read_timeout'] ?? $timeout;
                $persistent = $this->options['persistent'] ?? false;
                $password = $this->options['password'] ?? '';
                $args = [null, $this->options['host'], $timeout, $read_timeout, $persistent];
                if ($password) {
                    $args[] = $password;
                }
                
                $this->handler = new \RedisCluster(...$args);
                if (empty($this->options['prefix'])) {
                    $this->options['prefix'] = 'rcmaker_';
                }
            ...
            if(is_array($this->options['host'])){
                ...
                $this->handler = new \Predis\Client($host, $params);
            }
            /*支持集群*/
            else{
