...
abstract class Builder
{
    ...
    protected function parseData(Query $query, array $data = [], array $fields = [], array $bind = []): array
    {
        ...
        foreach ($data as $key => $val) {
            ...
            if (false !== strpos($key, '->')) {
                ...
                // 过滤非标量数据
                if(!$bind){
                   $result[$item] = is_string($val) ? "'$val'" : $val;
                }else{
    ...
    protected function parseWhereLogic(Query $query, string $logic, array $val, array $binds = []): array
    {
        ...
        foreach ($val as $value) {
            ...
            if ($value instanceof Closure) {
                ...
                $where[] = $test = ' ' . $logic . ' ' . $this->parseWhereItem($query, $field, $value, $binds);
            }
        }
        return $where;
    }
    ...
    protected function parseLike(Query $query, string $key, string $exp, $value, $field, int $bindType, string $logic): string
    {
        ...
        if (is_array($value)) {
            ...
            $whereStr = $key . ' ' . $exp . ' ' . ($query->getConnection()!==null ? $value : "'$value'");
        }
    ...
    protected function parseCompare(Query $query, string $key, string $exp, $value, $field, int $bindType): string
    {
        ...
        if(strpos((string)$value,'ThinkBind')){
            return $key . ' ' . $exp . ' ' .$value;
        }

        return $key . ' ' . $exp . ' ' . (is_string($value) ? "'$value'" : $value);
    }
    ...
    protected function parseDateTime(Query $query, $value, string $key, int $bindType)
    {
        ...
        $type = $query->getConnection()!==null ? $query->getFieldType($key) : 'int';
